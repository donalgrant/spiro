...
n_figs=16
dither_width=linspace(pi/60,pi/25,n_figs)
...
#    print(f'{k}: flat/eccen={flat}/{eccen}; ne={ne}, first={first}')
    dither=np.random.standard_normal(ne)*dither_width[k]
    major =[ 16/(T.neighbor_dist(j))**0.1         for j in arange(first,first+ne) ]
    orient=[ -T.direction(j)+pi/7+dither[j-first] for j in arange(first,first+ne) ]
